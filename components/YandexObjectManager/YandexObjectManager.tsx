...
export const YandexObjectManager: FC<Props> = props => {
    ...
    /**
     * Синхронизация открыт/закрыт с React'ом
     */
    useEffect(() => {
        // Если не выбран ни один объект
        if (objectIdProp === null) {
            return;
        }

        const object = objectManager.objects.getById(objectIdProp);

        // Если по какой-то причине объекта нет на карте
        if (object == null) {
            console.warn(
                `[YandexObjectManager] Не удалось найти объект #${objectIdProp} на карте`
            );
            return;
        }

        objectManager.objects.balloon.open(objectIdProp);
    }, [objectIdProp]);
